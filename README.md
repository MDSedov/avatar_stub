...
## Маленькая генерация через HTTP

Для отладки можно сгенерировать небольшой пул через endpoint:

```bash
curl -X POST "http://localhost:7070/admin/prepare?count=1000"
```

По умолчанию HTTP-генерация ограничена `10000`, чтобы случайно не запустить генерацию 230 000 файлов во время теста.
Для полного объема лучше использовать режим `generate`.
